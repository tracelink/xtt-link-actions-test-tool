export function sendDataOutbound (linkActionContext, outboundData) {
  const netsuiteRecordType = 'salesOrder'
  const JSONPath = getJSONPath()

  const uri = `${linkActionContext.config.url}/${netsuiteRecordType}`
  const header = { 'Content-Type': 'application/json' }
  let outboundDataJson = JSON.parse(outboundData);
  fetchAndMergeIdentifiers(linkActionContext, outboundDataJson)
  const outboundDataString = JSON.stringify(outboundDataJson)
  const response = linkActionContext.api.oAuth2.post(linkActionContext, uri, header, outboundDataString)

  if (response.statusCode == '204' && response.headers.hasOwnProperty('location')) {
    return { success: true, responseString: response.statusCode+'', errorString: '' }
  } else {
    const errorStr = 'Outbound failed : ' + response.headers['x-netsuite-propertyvalidation']
    linkActionContext.api.log(linkActionContext, errorStr)
    return { success: false, responseString: 'Failed', errorString: errorStr }
  }

  function fetchAndMergeIdentifiers (linkActionContext, outboundData) {
    /** idConfig
         * jsonPath is where id will be added
         * recordType is netsuite record type
         * whereCondition is filter criteria (It must return single object)
         * value is a jsonpath of value which will be replaced in whereCondition
         */
    const idConfigs = [
      {
        jsonPath: '$.currency',
        recordType: ['currency'],
        whereCondition: 'symbol+is+$symbol',
        value: { $symbol: '$.currency.symbol' }
      },
      {
        jsonPath: '$.terms',
        recordType: ['term'],
        whereCondition: 'name+is+$name',
        value: { $name: '$.terms.name' }
      },
      {
        jsonPath: '$.item.items[*].item',
        recordType: ['inventoryItem', 'assemblyItem', 'kitItem', 'discountItem', 'markupItem', 'subtotalItem', 'descriptionItem', 'paymentItem', 'shipItem', 'downloadItem', 'giftCertificateItem', 'subscriptionPlan', 'nonInventorySaleItem', 'nonInventoryResaleItem', 'nonInventoryPurchaseItem', 'serviceSaleItem', 'serviceResaleItem', 'servicePurchaseItem', 'otherChargeSaleItem', 'otherChargeResaleItem', 'otherChargePurchaseItem'],
        whereCondition: 'itemId+is+"$itemId"',
        value: { $itemId: '$.item.items[*].item.refName' }
      }
    ]
    for (const idConfig of idConfigs) {
      // fetch id
      const obj = JSONPath({ path: idConfig.jsonPath, json: outboundData })

      for (let i = 0; i < obj.length; i++) { // multiple items for jsonpath
        if (isIdExist(obj[i])) {
          continue
        }
        const whereCondition = getWhereCondition(linkActionContext, idConfig, outboundData, i)
        for (const rt of idConfig.recordType) {
          const id = fetchObject(linkActionContext, rt, outboundData, whereCondition)

          if (id) {
            // merge id
            const tempJsonPath = idConfig.jsonPath.replace('*', i)
            mergeObject(linkActionContext, tempJsonPath, outboundData, id)
            break
          }
        }
      }
    ...
    function fetchObject (linkActionContext, recordType, outboundData, whereCondition) {
      const uri = `${linkActionContext.config.url}/${recordType}?q=${whereCondition}`
      const header = { 'Content-Type': 'application/json' }
    ...
    function mergeObject (linkActionContext, jsonPath, outboundData, id) {
      const updater = (cb) => (value, _, { parent, parentProperty }) => {
        return parent[parentProperty] = cb(value)
      }
      const obj = JSONPath({ path: jsonPath, json: outboundData })[0]
      obj.id = id
      ...
      JSONPath({
        ...
        path: jsonPath,
        resultType: 'all',
        callback: updater((value) => obj)
      })
    }

    function getWhereCondition (linkActionContext, idConfig, outboundData, i) {
      let whereCondition = idConfig.whereCondition
      for (const key in idConfig.value) {
        const val = JSONPath({ path: idConfig.value[key], json: outboundData })[i]
        whereCondition = whereCondition.replace(key, val)
      }
      return whereCondition
    }
  }

  function isIdExist (obj) {
    const flag = obj.id || obj.externalId
    return flag
  }
  ...
  unicorn/prefer-at */
    ...
    /**
         ...
         * @return {GenericArray} Copy of the original array
         */
    ...
    JSONPath.prototype._trace = function (
      ...
      const loc = expr[0]; const x = expr.slice(1)
    ...
    JSONPath.prototype._slice = function (
      ...
      const len = val.length; const parts = loc.split(':')
      const step = (parts[2] && Number.parseInt(parts[2])) || 1
      let start = (parts[0] && Number.parseInt(parts[0])) || 0
      let end = (parts[1] && Number.parseInt(parts[1])) || len
    ...
    JSONPath.toPathString = function (pathArr) {
      const x = pathArr; const n = x.length
      let p = '$'
      for (let i = 1; i < n; i++) {
        if (!(/^(~|\^|@.*?\(\))$/u).test(x[i])) {
    ...
    JSONPath.toPointer = function (pointer) {
      const x = pointer; const n = x.length
      let p = ''
      for (let i = 1; i < n; i++) {
        if (!(/^(~|\^|@.*?\(\))$/u).test(x[i])) {
          p += '/' + x[i].toString()
